...
function StudentComponents() {
    ...
    const AddStudent = (e) => {
        ...
        if (studentId) {
            ...
            connection.patch(`/${studentId}/`, sendData).then((res) => {
                getStudents();
                setName('');
                setAddress('');
                setPhone('');
            ...
            let sendData = {name, address, phone};
            connection.post('/', sendData).then((res) => {
                getStudents();
                setName('');
                setAddress('');
                setPhone('');
                ...
                {students.map((student, index) => {
                    return (
                        <tr key={index}>
                            ...
                            <td>{student.phone}</td>
                            <td>
                                <button id="edit_button" onClick={() => getByCriteria(student.id)}>Edit</button>
                                <button id="button_delete" onClick={() => deleteStudent(student.id)}>Delete</button>
                            </td>
